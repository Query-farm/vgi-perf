# CI: the vgi-perf worker integration suite
...
(`test/sql/*.test`) against the vgi-perf VGI worker through the **real DuckDB
...
| `TRANSPORT`  | `VGI_PERF_WORKER` (the ATTACH LOCATION) | how the worker is launched |
|--------------|------------------------------------------|----------------------------|
| `subprocess` | `…/target/release/perf-worker`          | DuckDB spawns the stdio binary (default) |
| `http`       | `http://127.0.0.1:<port>`                | `perf-worker --http` (auto port; prints `PORT:<n>` on stdout, which the script polls for) |
| `unix`       | `unix:///tmp/perf.<pid>.sock`           | `perf-worker --unix <sock>` (prints `UNIX:<sock>` on stdout + creates the socket; the script waits for both) |
...
1. **Build the worker** — `cargo build --release --bin perf-worker`. The
   compiled `target/release/perf-worker` is a self-contained stdio worker the
   extension spawns (the `.test` files `ATTACH` it via `${VGI_PERF_WORKER}`).
...
3. **Preprocess** — the standalone runner links none of the extensions the
   ...
   through untouched. (The vgi-perf `.test` files already `LOAD vgi;`
   explicitly and use `require-env VGI_PERF_WORKER`, so the `require`-rewrite is
...
4. **Run** — [`run-integration.sh`](run-integration.sh) brings up the worker for
   the selected `TRANSPORT` and sets `VGI_PERF_WORKER` accordingly, stages the
...
WORKER_BIN="$PWD/target/release/perf-worker" \
TRANSPORT=subprocess \
  ci/run-integration.sh

# HTTP leg: launches `perf-worker --http`, reads PORT:<n>, attaches http://…
HAYBARN_UNITTEST=/path/to/haybarn-unittest WORKER_BIN="$PWD/target/release/perf-worker" \
  TRANSPORT=http ci/run-integration.sh

# Unix leg: launches `perf-worker --unix /tmp/perf.<pid>.sock`, attaches unix://…
HAYBARN_UNITTEST=/path/to/haybarn-unittest WORKER_BIN="$PWD/target/release/perf-worker" \
